...
*/
    ...
    public void addMoveAwayChance(final int benefit, final int inOrderNr, final Move m) {
        ...
        addChanceLowLevel(benefit,inOrderNr, myPos);   // stored as normal chance, but only at the piece origin.
    }
    ...
    public void addChance(final int benefit, final int chanceFutureLevel, int target) {
        if (chanceFutureLevel>MAX_INTERESTING_NROF_HOPS
            ...
            return;
        addChanceLowLevel(benefit, chanceFutureLevel, target);

        // add counter moves for all first move options to here
        Set<Move> firstMovesToHere = getFirstMovesWithReasonableShortestWayToHere();
        assert(firstMovesToHere!=null);
        for (Move m : firstMovesToHere) {
                        ...
                        for (VirtualPieceOnSquare opponentAtLMO : opponentAtTarget.getShortestReasonableUnconditionedPredecessors()) {
                            ...
                            */
                            ...
                            if (abs(defendBenefit) > 1) {
                                ...
                                opponentAtLMO.addChanceLowLevel(defendBenefit, finalFutureLevel, target); //max(inFutureLevel, defendInFutureLevel));
                            }
                        }
                    }
                }
     ...
     * @param benefit
     * @param inOrderNr
     * --- still needed? careful, is still in addChance for firstMovesToHere fashion
